...
* indentations for make element of tag have a block of children, and
  children is inside of the element of tag if the indetations have more 
  spaces with the parent tag.
* Auto closing will be added in the result document if indentations 
  command are less spaces.
* Tags formating (%) have character that will represent an ID(#), CLASS(.),
  Attribute({}), Ruby Code(=), and Ruby String(==) 
...
line inside block of indentation (children of) will not be parse this is usefull
for some block no need to parse.